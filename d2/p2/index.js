const { readFileSync } = require("fs");
const input = readFileSync('./input.txt', 'utf8').trim().split("\r\n");

let totalSum = 0n;

const ranges = input[0].split(",")
.map(rangeStr => {
    const parts  = rangeStr.split('-');
    const start = BigInt(parts[0]);
    const end = BigInt(parts[1]);
    return {start, end};
})

let maxEndId = 0;
for (const range of ranges) {
    if(range.end > maxEndId) {
        maxEndId = range.end;
    }
}

const countedId = new Set();

for (let Len = 1; Len <= 5; Len++) {
    for (let k = 2; k <= Math.floor(10 / Len); k++) {
        let Xmin = 10n ** BigInt(Len -1);
        let Xmax = (10n ** BigInt(Len)) - 1n

        for (let X = Xmin; X <= Xmax; X++) {
            const xStr = X.toString()
            const invalidIdStr = xStr.repeat(k);
            const invalidId = BigInt(invalidIdStr);

            if(invalidId > maxEndId) {
                break;
            }
            if(countedId.has(invalidId)) {
                continue;
            }

            for (const range of ranges) {
                if(invalidId >= range.start && invalidId <= range.end) {
                    totalSum += invalidId;
                    countedId.add(invalidId)
                    break;
                }
            }
        }

    }
}

console.log(totalSum);

